% ICOADS_Step_04_Neighbor_std
%
%  For each month, the standard deviation of SST computed in
% "ICOADS_Step_03_WM" are pulled and averaged across all available years
...
function ICOADS_Step_04_Neighbor_std(varname)

    % Set direcotry of files  ---------------------------------------------
    dir_load  = ICOADS_OI('WM');
    dir_save  = ICOADS_OI('Mis');
    ...
    for mon = 1:12
        ...
        for yr = 1850:2014
            ...
            file = ['IMMA1_R3.0.0_',num2str(yr),'-',cmon,'_WM_',varname,'.mat'];
            clear('ST','NUM');
