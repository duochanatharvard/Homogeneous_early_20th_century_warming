...
function [out,lme,M] = HM_lme_fit_hierarchy(file_load,do_sampling,target,do_hierarchy,do_hierarchy_random)
    ...
    if target == 0,
        ...
        if do_hierarchy == 0,
            ...
            [unique_nat,~,J_nat] = unique([kind_cmp_1(:,1:2);kind_cmp_2(:,1:2)],'rows');
            J_nat_1 = J_nat(1:N_pairs);
            J_nat_2 = J_nat(N_pairs+1:end);
            N_nat = size(unique_nat,1);

            [W_out,L] = HM_lme_build_hierarchy(W_X,BINNED);

            M_nat = HM_lme_matrix(N_pairs,N_nat,J_nat_1,J_nat_2,W_out(:,1:N_nat),data_cmp,weigh_use,...
                          do_region,do_season,do_decade,group_region,group_season,group_decade);

            M_dck = HM_lme_matrix(N_pairs,N_groups,J_grp_1,J_grp_2,W_out(:,1+N_nat:end),data_cmp,weigh_use,...
                          do_region,do_season,do_decade,group_region,group_season,group_decade);

            % This function merges the matrice at nation level and deck deck_level
            % given the information of grouping and deck in L
            M = HM_lme_matrix_hierarchy(M_nat,M_dck,L,do_hierarchy_random,do_region,do_season,do_decade);
    ...
    if target == 0,
        ...
        if do_hierarchy == 0,
            ...
            out = HM_lme_post_hierarchy(M,lme,unique_nat,unique_grp,...
                              do_hierarchy_random,N_nat,N_groups,do_sampling,...
                              do_region,do_season,do_decade);

            out = HM_lme_post_mask_effects(out,M_dck,...
                              do_region,do_season,do_decade,do_hierarchy_random);
        end
    end
    % !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    % To be done, adding relative to a nation function !!
